...
setErrorMessage(message)
  ...
  return (
    <div>
        ...
        <form onSubmit={(e)=>e.preventDefault()}className='absolute bg-black w-3/12 p-12 my-36 mx-auto left-0 right-0 bg-opacity-80 rounded-lg'>
            <h1 className='text-white font-bold text-3xl py-4'>{isSignInForm ?"Sign In":"Sign Up"}</h1>
            {!isSignInForm &&(
            <input ref={name} type="text" placeholder='Full Name' className='p-4 my-4 w-full bg-zinc-800 text-zinc-400 cursor-pointer'/>
            )}
            <input ref={email} type="text" placeholder='Email or Phone number' className='p-4 my-4 w-full bg-zinc-800 text-zinc-400 cursor-pointer '/>

            <input ref={password}type="password" placeholder='Password' className='p-4 my-4 w-full bg-zinc-800 text-zinc-400 cursor-pointer'/>
            <p className='text-red-500'>{errorMessage}</p>
            <button className='bg-red-500 text-white p-4 w-full my-6 cursor-pointer' onClick={handleButtonClick}>{isSignInForm ?"Sign In":"Sign Up"}</button>
